...
class Coupon extends CI_Controller {
	...
	private $mName = 'couponmodel';
	private $page_title = 'Manage Coupons';
	private $add_title = 'Add Coupon';
	private $edit_title = 'Edit Coupon';
	private $PNotify_title = 'Coupon';
	private $add_msg = 'Coupon Added Successfully';
	private $edit_msg = 'Coupon has been successfully Updated';
	...
	public function process(){
		...
		$chktoken = $this->{$this->mName}->check_coupon($token);
		if($chktoken == 0){
			$data['token'] = $token;
		}else{
			$data['token'] = '';
			echo "<script>alert('Please refresh your page for security issues. No token generated');</script>";
		}
